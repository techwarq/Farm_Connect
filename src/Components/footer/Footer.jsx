...
export default function Footer() {
    ...
    const loadTidioWidget = () => {
        const tidioScript = document.createElement('script');
        tidioScript.src = `//code.tidio.co/i70k0nkbormtjtecvzs5dzykn6njosqk.js`;
        tidioScript.async = true;
        document.body.appendChild(tidioScript);

        // Open Tidio chat after the script has loaded
        tidioScript.onload = () => {
    ...
    useEffect(() => {
        // Load Tidio Widget script after the component mounts
        loadTidioWidget();
    }, []);
    return (
            ...
            <div className="bg-gray-200" style={{ backgroundColor: mode === 'dark' ? 'rgb(55 57 61)' : '', color: mode === 'dark' ? 'white' : '', }}>
                <div className="container px-5 py-3 mx-auto flex items-center sm:flex-row flex-col">
                    <Link to={'/'} className='flex'>
                        <div className="flex ">
                            <h1 className=' text-2xl font-bold text-black  px-2 py-1 rounded' style={{ color: mode === 'dark' ? 'white' : '', }}>Farm<span className=' text-green-400'>Connect</span></h1>
                        </div>
                    </Link>
                    <p className="text-sm text-gray-500 sm:ml-6 sm:mt-0 mt-4" style={{ color: mode === 'dark' ? 'white' : '' }}>© 2023 FarmeConnect —
                        <a href="https://twitter.com/knyttneve" rel="noopener noreferrer" className="text-gray-600 ml-1" target="_blank" style={{ color: mode === 'dark' ? 'white' : '' }}>www.farmConnect.com</a>
                    </p>
